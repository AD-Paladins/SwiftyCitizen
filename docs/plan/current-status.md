...
- Flashcards now let the learner write their own answer before revealing: `FlashcardState.recordAnswer(_:)` stores it in `currentAnswer`, `assess()` captures it into `FlashcardAttemptRecord.answerText`, and it persists to `QuestionAttempt.answerText` (reusing the mock-test-only field, no schema change). On reveal, `AnswerCard(grading:false)` shows "Your answer" and the "Official answer" side by side with no automated verdict, preserving the self-assessment-only principle; a seeded `userAnswers` entry (missed-question review) still wins over the live answer. Covered by tests in `StudyLogicTests`/`StudyDomainTests`.
- Redefined the flashcard `Again` button (it previously advanced like `Hard`/`Got it`, which contradicted its "show again" label). Now `assess(.again)` re-presents the same question for a fresh attempt without advancing or recording an attempt; only `Hard`/`Got it` mark the card and advance, so the final confirmation is what counts. Retries do not pollute `gotItRate` and an unconfirmed card stays `Due` in Targeted Review. The old linear assumption was removed from `flashcardStateProgressesThroughDeck`. Covered by `flashcardStateAgainRePresentsWithoutAdvancing`/`flashcardStateRecordsOnlyFinalConfirmation` (142 tests pass).
