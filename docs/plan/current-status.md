...
- The mock test should support selection-based answers (single-select and multi-select), not only typed text. Slice A (tiles + fallback, no distractors) is scoped and specified in `docs/plan/mock-test-selection-spec.md`; multi-select distractors are deferred to a follow-up iteration.
